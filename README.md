...
> **Status:** v1 complete (M1–M7). Requirements ([`SRS.md`](./SRS.md)) and architecture ([`ARCHITECTURE.md`](./ARCHITECTURE.md)) baseline. The Next.js app boots a Mapbox map with live GPS vessel position, a 54-boat IndexedDB library, NOAA tide / wave overlays, OSM marina overlay with amenity-rich detail cards, long-press waypoint drop with great-circle routing and adaptive ETA, and a fuel-range ring. PWA installable with a manifest + SVG icon, a hand-rolled service worker caches the app shell, Mapbox tiles, and last-good API responses, and a side-menu "Save current view offline" button warms the tile cache for the visible bbox. A first-launch disclaimer covers NFR-033; an offline badge surfaces when the network drops. Keyboard accessibility (Escape closes modals) is in place across the side menu, bottom sheet, seed picker, and disclaimer.
